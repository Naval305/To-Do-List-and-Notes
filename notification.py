...
def schedule(message, hour, mins, day, month, year, priority):
    ...
    if priority == 1:
        ...
        if in_sec >= 2592000:
            scheduler.enter(
                in_sec - 30 * 24 * 60 * 60, 1, notif_schedule, ("1 month left",)
            )
            ...
            if in_sec >= 86400:
                scheduler.enter(
                    in_sec - 24 * 60 * 60, 1, notif_schedule, ("1 day left",)
                )
                ...
                if in_sec >= 3600:
                    scheduler.enter(
                        in_sec - 15 * 60, 1, notif_schedule, ("15 mins left",)
                    )
                    scheduler.enter(in_sec, 1, notif_schedule, (" ",))
                else:
                    if in_sec > 300:
                        scheduler.enter(
                            in_sec - 5 * 60, 1, notif_schedule, ("5 mins left",)
                        )
                        scheduler.enter(in_sec, 1, notif_schedule, (" ",))
                    else:
                        scheduler.enter(in_sec, 1, notif_schedule, (" ",))
        ...
        if in_sec > 2592000:
            scheduler.enter(
                in_sec - 30 * 24 * 60 * 60, 1, notif_schedule, ("30 days left",)
            )
            scheduler.enter(
                in_sec - 15 * 24 * 60 * 60, 1, notif_schedule, ("15 days left",)
            )
            scheduler.enter(in_sec - 24 * 60 * 60, 1, notif_schedule, ("1 day left",))
            scheduler.enter(in_sec - 60 * 60, 1, notif_schedule, ("1 hour left",))
            scheduler.enter(in_sec, 1, notif_schedule, (" ",))
            ...
            if in_sec > 86400:
                ...
                scheduler.enter(
                    in_sec - 12 * 60 * 60, 1, notif_schedule, ("12 hour left",)
                )
                scheduler.enter(in_sec - 60 * 60, 1, notif_schedule, ("1 hour left",))
                scheduler.enter(in_sec, 1, notif_schedule, (" ",))
            else:
                if in_sec > 3600:
                    ...
                    scheduler.enter(
                        in_sec - 30 * 60, 1, notif_schedule, ("30 mins left",)
                    )
                    scheduler.enter(in_sec, 1, notif_schedule, (" ",))
                else:
                    if in_sec > 1800:
...
def notificationn():
    with open(
        os.path.join(os.getcwd(), "database_files", "tasks_db.txt"), "r"
    ) as database:
