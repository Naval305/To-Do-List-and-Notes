...
class ToDoList(MDApp):
    ...
    def task_content(
        self, source
    ):  # to send the task, desc, date, time, priority to be shown in widget(MDCard)
        self.source = source
        ...
        if self.source == "user":
            ...
            if self.condition == "True":
                self.spawn_program(["python", "notification.py"])
    ...
    def notes_content(
        self, cond
    ):  # to send the notes(text) to be shown in widget(MDCard)
        self.cond = cond

        if self.cond == "pass":
            self.card2 = self.root.ids.box2
            self.card2.add_widget(
                Show_card_Notes(app, self.text, self.current_date_time)
            )
            ...
            for self.one in self.gdata:
                ...
                self.current_date_time = self.one[1] if len(self.one[0]) > 0 else ""
                self.card2.add_widget(
                    Show_card_Notes(app, self.text, self.current_date_time)
                )
    ...
    def restart_delete_content(self):  # to delete all file conetent and restart app
        ...
        task_file.close()
        self.stop()
        self.spawn_program(["python", "main.py"])
    ...
    def checkPASS(self):
        p_file = open(os.path.join(os.getcwd(), "database_files", "password.txt"), "r")
    ...
    def on_start(self):
        ...
        theme_f = open(os.path.join(os.getcwd(), "database_files", "theme.txt"), "r")
        ...
        try:
            ...
            Snackbar(
                ...
                buttons=[
                    MDIconButton(
                        icon="information", on_release=lambda x: self.quote_info()
                    ),
                ],
    ...
    def theme(self):  # to set theme as automatic or not
        theme_file = open(os.path.join(os.getcwd(), "database_files", "theme.txt"), "r")
        theme_current = theme_file.read()

        if theme_current == "On":
            theme_write = open(
                os.path.join(os.getcwd(), "database_files", "theme.txt"), "w"
            )
    ...
    def unhide(self):  # to check if password is correct or not
        ...
        password_file = open(
            os.path.join(os.getcwd(), "database_files", "password.txt"), "r"
        )
    ...
    def save_pass(self):  # to save new pass in file
        ...
        if self.new_pass == self.confirm_pass:
            new_pass_file = open(
                os.path.join(os.getcwd(), "database_files", "password.txt"), "a"
            )
    ...
    def image_path(self, image):
        return os.path.join(os.getcwd(), "image_files", image)
    ...
    def delete_finished(self):  # dialog to delete finished tasks list
        self.finished_dialog = MDDialog(
            ...
            buttons=[
                MDFlatButton(
                    text="CANCEL", on_release=lambda x: self.finished_dialog.dismiss()
                ),
                MDRaisedButton(
                    text="DELETE",
                    md_bg_color=get_color_from_hex("FF0000"),
    ...
    def deletefinished(self):  # deletion of finished tasks list
        finish = open(
            os.path.join(os.getcwd(), "database_files", "finished_tasks_db.txt"), "w"
        )
