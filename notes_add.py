...
class NotesContent(Popup):
    def __init__(self, app, **kwargs):
        ...
        self.button = MDFillRoundFlatButton(
            text="SAVE", pos_hint={"top": 0.1, "center_x": 0.5}
        )
        self.button.bind(
            on_press=lambda x: self.app.Notes(self.textt.text, current_date_time)
        )
        self.button.bind(on_release=lambda x: self.dismiss())
...
class Notes_edit(Popup):
    def __init__(self, app, text, current_date_time, **kwargs):
        ...
        self.button.bind(
            on_press=lambda x: self.app.Notes(self.textt.text, self.current_date_time)
        )
        self.button.bind(on_release=lambda x: self.dismiss())
