...
class Card(MDCard):  # adding Card with task and other deatils, also including widgets
    def __init__(self, app, task, desc, time, date, priority, **kwargs):
        ...
        self.data = GetData(  # sending data to GetData class to store it in file
            self.task, self.desc, self.time, self.date, self.priority
        )
        ...
        self.box = MDBoxLayout(  # box layout to hold the content as buttons etc.
            ...
            size_hint=(0.4, 0.20),
            pos_hint={"top": 0.88, "x": 0.74},
            spacing=7,
            padding=8,
        )

        self.task_txt = MDExpansionPanel(  # to store task, description as well as date
            content=TwoLineListItem(text=self._temp_time, secondary_text=date),
            panel_cls=MDExpansionPanelTwoLine(text=self.task, secondary_text=self.desc),
        ...
        self.done.bind(
            on_press=lambda x: self.app.delete_task(
                self.task, self.desc, self.time, self.date, self.priority, "done"
            )
        )
        ...
        self.delete.bind(
            on_press=lambda x: self.app.delete_task(
                self.task, self.desc, self.time, self.date, self.priority, "delete"
            )
        )
        ...
        self.line_lbl = MDLabel(
            text="Click here to expand", pos_hint={"x": 0.728, "y": 0.065}
        )
        ...
        if self.priority != "" and (
            int(self.priority) == 2
            or int(self.priority) == 3
            or int(self.priority) == 4
        ):
