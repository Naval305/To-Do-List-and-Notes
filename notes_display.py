...
class Show_card_Notes(MDCard):
    def __init__(self, app, text, current_date_time, **kwargs):
        ...
        self.data = GetNotes(self.text, self.current_date_time)
        self.data.getdata()

        self._date_time = (
            self.current_date_time[0:2]
            + " "
            + self.current_date_time[2:5]
            + " "
            + self.current_date_time[5:9]
            + ", "
            + self.current_date_time[9:12]
            + ",  "
            + self.current_date_time[12:14]
            + ":"
            + self.current_date_time[14:16]
            + " "
            + self.current_date_time[16:18]
        )

        self.upper_btns = MDBoxLayout(
            orientation="vertical",
            size_hint=(0.4, 0.20),
            pos_hint={"top": 0.25, "x": 0.9},
            spacing=14,
            padding=8,
        )
        ...
        self.notes_text.bind(
            on_double_tap=lambda x: self.app.read_notes_popup(
                self.notes_text.text, self.current_date_time
            )
        )
        ...
        self.delete.bind(on_release=lambda x: self.app.anime4(self))
        self.upper_btns.add_widget(self.delete)

        self.label = MDLabel(
            text=self._date_time, size_hint=(0.5, 0.2), pos_hint={"x": 0.04, "y": 0.001}
        )
