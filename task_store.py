...
class GetData:
    ...
    def store_data(self):
        ...
        with open(
            os.path.join(os.getcwd(), "database_files", "tasks_db.txt"), "a"
        ) as db1:
            ...
            if len(self.data_lines) != 0:
                for line in self.data_lines:
                    ...
                    if (
                        line
                        == f"{self.task};{self.desc};{self.time};{self.date};{self.priority}"
                    ):
                ...
                if len(self.descision) == 0:
                    db1.write(
                        f"ID:{self.idd}>{self.task};{self.desc.strip()};{self.time};{self.date};{self.priority}\n"
                    )
                else:
                    pass
            if len(self.data_lines) == 0:
    ...
    def delete_data(self, task, desc, time, date, priority):
        ...
        with open(
            os.path.join(os.getcwd(), "database_files", "tasks_db.txt"), "w"
        ) as file_2:
            file_2.write("")
